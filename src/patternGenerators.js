...
export const generateNeurons = (settings) => {
  const { 
    width, 
    height, 
    strokeWidth = 1.5, 
  ...
  const generateBranch = (startX, startY, angle, length, generation, parentColor) => {
    ...
    const angleVariation = (Math.random() - 0.5) * Math.PI * curvature;
    ...
    const cp1Angle = angle + (Math.random() - 0.5) * 0.3;
    ...
    const cp2Angle = finalAngle + (Math.random() - 0.5) * 0.3;
    ...
    const shouldBranch = Math.random() < branchProbability && generation < depth;
    
    if (!shouldBranch || generation === depth) {
      // Terminal node - draw a dot
      const currentNodeSize = nodeSize * Math.pow(0.85, generation);
      paths += `<circle cx="${endX}" cy="${endY}" r="${currentNodeSize}" fill="${parentColor}" opacity="${opacity}"/>`;
    }
    
    // Continue branching
    if (shouldBranch) {
      const numSubBranches = generation === 0 ? 2 : (Math.random() > 0.5 ? 2 : 3);
      const newLength = length * (0.6 + Math.random() * 0.2);
      const angleSpread = (Math.PI / 3) * spread;
      
      for (let i = 0; i < numSubBranches; i++) {
        const branchAngle = finalAngle + (i - (numSubBranches - 1) / 2) * angleSpread + (Math.random() - 0.5) * 0.4;
  ...
  for (let i = 0; i < branches; i++) {
    ...
    const initialLength = Math.min(width, height) * 0.15 * (0.8 + Math.random() * 0.4);
  ...
  // Add central node
  paths += `<circle cx="${centerX}" cy="${centerY}" r="${nodeSize * 1.5}" fill="${color}" opacity="${opacity}"/>`;
